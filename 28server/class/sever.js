...
var Sever = function(_config){
	...
	　　var j = schedule.scheduleJob(rule, function(){
			...
			}else if (self.state == gameConfig.gameState.downTimeEnd){
				if (self.downEndTime <= 0){
					self.sendCard();
				}else{
					--self.downEndTime;
				}
			}else if(self.state == gameConfig.gameState.open){
				if (self.openTime <= 0){
					self.openEnd();
				}else{
					--self.openTime;
			...
			}else if(self.state == gameConfig.gameState.openEnd){
				if (self.openEndTime <= 0){
					self.sendCoin();
				}else{
					--self.openEndTime;
				}
			}else if(self.state == gameConfig.gameState.sendCoin){
				if (self.sendCoinTime <= 0){
					self.noting();
				}else{
					--self.sendCoinTime;
				}
			}else if(self.state == gameConfig.gameState.noting){
			...
			}else if(self.state == gameConfig.gameState.sendCard){
				if (self.sendCardTime <= 0){
					self.open();
				}else{
					--self.sendCardTime;
				}
	...
	this.sendCard = function(){
		console.log("发牌")
		this.sendCardTime = this.sendCardTimeMax;
		this.state = gameConfig.gameState.sendCard;
