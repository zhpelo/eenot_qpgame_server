...
var GameInfo = function(){
	...
	var Game = function(){
			...
			for(var i = 0 ;i < this.upUserList.length; i++){
				if (this.upUserList[i].length > 0){
					...
					for (var userItem in this.downCoinList[i]){
						...
						for(var z = 0 ; z < 3 ; z++){
							if (Result.jieguo[z] > 0){
								...
								zhuangjiaWin += (-Value);
								//返回本金
								userWin += (cunValue + (this.downCoinList[i][userItem][z] * 8));
								cunWin += cunValue;
								tax += Value - cunValue;
							}else{
								var Value = this.downCoinList[i][userItem][z] * Result.jieguo[z];
								...
								zhuangjiaWin += (-Value);
								cunWin += Value;
								userWin += this.downCoinList[i][userItem][z] * (Result.jieguo[z] + 8);
						...
						isDownCoin = true;
						var logTemp = {userId:userItem,MatchId:this.matchId,downCoin:(open2+open3+open4),winCoin:cunWin,open2:open2,open3:open3,open4:open4,tax:tax,isBanker:0,serverId:gameConfig.serverId,tableid:i}
					...
					if (isDownCoin){
						...
						var logTemp = {userId:zhuangjia.userId,MatchId:this.matchId,downCoin:0,winCoin:zhuangjiaWin,open2:-open2ad,open3:-open3ad,open4:-open4ad,tax:zhuangtax,isBanker:1,serverId:gameConfig.serverId,tableid:i}
						CoinLog.push(logTemp);
		...
		this.downCoin = function(_socket,info){
			...
			var zhuanglist = this.upUserList[this.userList[_socket.userId].TableId];
			...
			if (Math.floor(zhuang.maxDownCoin) < gameConfig.coinConfig[info.chips] * 8){
				_socket.emit("downCoinResult", {ResultCode:-1,msg:"下注已经满"});
				return;
			}
			var youScore = this.userList[_socket.userId].getScore();
			//扣掉自己的钱
			if (!this.userList[_socket.userId].downCoin(gameConfig.coinConfig[info.chips] * 8)){
			...
			zhuang.maxDownCoin -= gameConfig.coinConfig[info.chips] * 8;
			//记录在什么门下下注多少
			var userInfo = {userid:_socket.userId,downCoin:gameConfig.coinConfig[info.chips],meng:info.selectId};
			if (this.downCoinList[this.userList[_socket.userId].TableId][_socket.userId]){
				this.downCoinList[this.userList[_socket.userId].TableId][_socket.userId][info.selectId] += gameConfig.coinConfig[info.chips];
			}else{
				this.downCoinList[this.userList[_socket.userId].TableId][_socket.userId] = [];
				this.downCoinList[this.userList[_socket.userId].TableId][_socket.userId][0] = 0;
				this.downCoinList[this.userList[_socket.userId].TableId][_socket.userId][1] = 0;
				this.downCoinList[this.userList[_socket.userId].TableId][_socket.userId][2] = 0;
				this.downCoinList[this.userList[_socket.userId].TableId][_socket.userId][info.selectId] = gameConfig.coinConfig[info.chips];
			}

			//做金钱记录
			var userInfolog = {userid:_socket.userId,score_before:youScore,score_change:(-gameConfig.coinConfig[info.chips] * 8),score_current:youNowScore,change_type:gameConfig.logflag,isOnline:true};
			this.score_changeLogList.push(userInfolog);
