...
Csocket.on('connected',function(msg){
    ...
    var info = {serverName:"八搭二游戏",serverId:gameConfig.serverId,signCode:"slel3@lsl334xx,deka"}
...
io.on('connection', function(socket){
  ...
  socket.on('down',function(){
    gameInfo.down(socket);
  })

  //下注
  socket.on('downCoin',function(info){
    ...
    try{
      ...
      log.warn('downCoin-json');
    }

    gameInfo.downCoin(socket,info);
  })

  //对决
  socket.on('vs',function(info){
    try{
      var data = JSON.parse(info);
      info = data;
    }
