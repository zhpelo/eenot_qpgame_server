...
var bet = [[2,5,20],[5,15,60],[10,30,160],[20,80,400],[50,200,1000],[0,0,2000],[0,0,0]];
...
check = function(reArray){
		...
		for (var i = 0 ; i < 9 ; i++){
			...
			for(j = 0 ;j < linesPoint[0].length ; ++j){
				...
				if (reArray[row][col] == sameCard || reArray[row][col] == 5){
					//console.log("reArray["+row+"]["+col+"]:" + reArray[row][col] + " sameCard:" + sameCard)
					++sameCardCount;
					
				}else{
					//不能再找到相同的
					if (sameCard == 5 && reArray[row][col] != -1){
						sameCard = reArray[row][col];
						++sameCardCount;
					}else{
						break;
...
var betAcd = [6000,5000,4000,3000,2000,1000,500];
var betRand = [];
var betTotal = 0;
var caishenCount = 0;
