FishUser = function(_userInfo){
	...
	this.x = 0;
	this.RandCount = 0;
	this.TotalCount = Math.floor(Math.random()*10000) + 2000;
	this.timer;
	this.again = Math.floor(Math.random()*60000);
	//this.again = 2;
	//离线后再次登录时间
	this.againMin = 2000;	//2s
	this.againMax = 500000;	//200s
	this._online = true;
	this._out = false;
	...
	this.initNetWork = function(){
	    ...
	    this._socket.on('ServerListResult',function(msg){
	    	...
	    	if (self._matchRoot){
	    		var idx = 0;
		        self._ip = msg.GameInfo.serverInfo.match[idx].ip;
		        self._port = msg.GameInfo.serverInfo.match[idx].prot;
	...
	this.shoot = function(){
		...
		if ((this.TotalCount <= 0 && this._online)){
			this.disconnect();
			if (!this._matchRoot){
				this.again = Math.floor(Math.random()* this.againMax) + this.againMin;	
			}else{
				this.again = Math.floor(Math.random()* (this.againMax / 4)) + this.againMin;	
			}
