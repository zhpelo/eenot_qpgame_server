	...
	this.initNetWork = function(){
	    ...
	    this._socket.on('ServerListResult',function(msg){
	     ...
	     //    self._port = msg.GameInfo[4].serverInfo.normal[idx].prot;
	    	...
	    	console.log(self._userId + "进入8da2游戏" + parseInt(self.idx + 1) +"倍房间!");
	        self._ip = msg.GameInfo[4].serverInfo.normal[self.idx].ip;
	        self._port = msg.GameInfo[4].serverInfo.normal[self.idx].prot;
	...
	this.DownCoin = function(){
		...
		if (this.TotalCount > 10){
			...
			if (!Math.floor(Math.random()*100)){
				...
				this._gameSocket.emit('vs',JSON.stringify({selectId:this.selectid}));
				return;
			}
