jingdiancowUser = function(_userInfo){
	//相关属性
	this._userId = 0;
	this.RandCount = 0;
	...
	this.initNetWork = function(){
	    ...
	    this._socket.on('ServerListResult',function(msg){
	    	...
	    	// var idx = 0;
	     //    self._ip = msg.GameInfo[8].serverInfo.normal[idx].ip;
	     //    self._port = msg.GameInfo[8].serverInfo.normal[idx].prot;

	    	console.log(self._userId + "进入经典牛牛游戏" + parseInt(self.idx + 1) +"倍房间!");
	        self._ip = msg.GameInfo[8].serverInfo.normal[self.idx].ip;
	        self._port = msg.GameInfo[8].serverInfo.normal[self.idx].prot;
