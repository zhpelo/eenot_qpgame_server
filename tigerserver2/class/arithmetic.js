...
arithmetic = function(_idx){
	...
	this.hit = function(_pro,_betidx,hitCount){
		...
		if (Math.floor((this.HitTimes[_pro][_betidx] / 10)) >= this.pro_max[_pro]){
			...
			//入彩池
			this.addScorePool(_betidx,Math.floor(betCount[_betidx] * 1000 * this.poolBet));
		}
	...
	this.getArray = function(_betIdx,_LotteryCount){
		...
		if (rand == 0 && _LotteryCount > 5000){
			...
			//检测实际实际奖池是否能发
			//console.log("this.getScorePool(betValue)" + this.getScorePool(_betIdx))
		...
		if (total_temp == 0){
  		...
  		if (total_temp >= 10){
  			...
  			//console.log(total_temp + "|" + (tempitem.freeCount + tempitem.scoreCount));
