	...
	var Game = function(){
		...
		this.init = function(_config,_fishConfig){
			...
			　　var j = schedule.scheduleJob(rule, function(){
					...
					// 	log.info("nickname:" + randlist[j]._nickname + " score:"  + randlist[j]._score);
					// }
		...
		this.updateUser = function(userInfo){
			...
			if (gameConfig.isMatchRoom){
				...
				var resultObj = {account:this.userList[userInfo._userId]._account,id:this.userList[userInfo._userId]._userId,nickname:this.userList[userInfo._userId]._nickname,score:this.userList[userInfo._userId]._score,bet:gameConfig.gameBet};
				result = {resultid:'1',msg:'login fishserver succeed!',Obj:resultObj};
				socketItem.emit('loginGameResult',result);
			}
		...
		this.balanceByUserId = function(_userId){
			...
			var CoinLog = [];
			var logTemp = {userId:_userId,useCoin:useCoin,winCoin:winCoin - useCoin,tax:0,serverId:gameConfig.serverId,gameId:gameConfig.gameId}
		...
		this.addRankUserList = function(_info){
			//log.info(_info);
			this.RankUserList[_info.userInfo.userId] = {id:_info.userInfo.userId,nickname:_info.userInfo._nickname,_score:gameConfig.roomApplyCoin};
		}
		...
		this.getFishList = function(_socket){
			...
			_socket.emit("getFishListResult",{ResultCode:0,msg:"",data:{fishList:fishList,SceneType:this.fishsever.getFishCurrentSceneType()}});
		}
		...
		this.fishHit = function(_userId,fishId,_bulletId,_sendId){
			...
			if(this.userList[_userId].getSeat() >= 0 && this.userList[_userId].getSeat() < this.fishsever.seatMax){
				...
				if (hitResult.propCount > 0){
					var info = {userId:_userId,proTypeId:hitResult.propId,addProCount:hitResult.propCount,roomid:gameConfig.serverId,typeid:3};
					//发送给游戏大厅
		...
		this.boomFishHit = function(_userId,boomFishId,fishIdList,_sendId){
			...
			var bet = this.fishsever.getBoomList(_userId,boomFishId)
			...
			if(this.userList[_userId].getSeat() >= 0 && this.userList[_userId].getSeat() < this.fishsever.seatMax){
				...
				for(var i = 0 ; i < 100 && fishIdList.length; ++i){
					...
					var hitResult = this.fishsever.boomFishHit(this.userList[_userId],bet,10,fishIdList[idx]);
					this.userList[_userId].winscore(hitResult.socre * gameConfig.gameBet);
				...
				this._io.sockets.in(tablestring).emit('boomFishHitResult', {ResultCode:0,ResultData:{userId:_userId,hitSocre:score,fishList:outfishList}});
				//log.info("总分:" + score);
				return score;
			}else{
				log.info("用户没有进入房间!")
